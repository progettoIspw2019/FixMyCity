...
public class CommunityReportDAO {
	...
	public List<CommunityReport> findAll() {
		/*
		 * OLD WAY
		 * Query<CommunityReport> queryResult = (Query<CommunityReport>)
		 * entityManager.createQuery("from CommunityReport"); List<CommunityReport>
		 * result = new ArrayList<CommunityReport>(queryResult.list());
		 * List<CommunityReportBean> resultBean = new ArrayList<CommunityReportBean>();
		 * 
		 * for (CommunityReport cp : result) {
		 * 
		 * // TODO il bean ha solo pochi dati in questo momento CommunityReportBean
		 * report = new CommunityReportBean(); report.setLatitude(cp.getLatitude());
		 * report.setLongitude(cp.getLongitude());
		 * report.setDescription(cp.getFullDescription()); resultBean.add(report);
		 * 
		 * }
		 */
